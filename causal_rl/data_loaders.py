...
class Trajectories(Dataset):
    ...
    def __init__(self, env: CausalEnv, steps: int, trajectory_size: int=10):
        ...
        self.steps = steps // trajectory_size
        self.states, self.collisions, _ = load_sim(self.env, self.steps + trajectory_size)
        self.states = self.states.view(-1, trajectory_size, self.states.shape[1], self.states.shape[2])
        self.collisions = self.collisions.view(-1, trajectory_size, self.collisions.shape[1], self.collisions.shape[2])
        self.bad_state_indices: Set[int] = set()
